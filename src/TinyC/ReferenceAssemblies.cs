...
// Finds the net10.0 reference-assembly folder so the compiler can bind against
// the framework's public surface (System.Runtime, System.Console, ...) rather
// than the live runtime's implementation assembly.
internal static class ReferenceAssemblies
{
    public static string LocateNet10()
    {
        foreach (var root in CandidateDotnetRoots())
        {
            ...
            // Highest version that actually carries a net10.0 ref folder.
        ...
        throw new TinyCompileException(
    ...
    {
        // 1) Directory of the running muxer (dotnet.exe).
