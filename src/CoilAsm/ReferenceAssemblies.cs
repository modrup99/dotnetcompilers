...
// Locates the net10.0 reference-assembly folder so the assembler binds against the
// framework's public surface (System.Runtime, System.Console, ...) rather than the
// live runtime's implementation assembly — making emitted assemblies referenceable
// from C#/VB.NET.
internal static class ReferenceAssemblies
{
    public static string LocateNet10()
    {
        ...
        throw new CoilAsmException("could not locate net10.0 reference assemblies");
    }
    ...
    private static IEnumerable<string> CandidateDotnetRoots()
    {
        ...
        if (proc is not null)
        {
            ...
            if (dir is not null && File.Exists(Path.Combine(dir, "dotnet.exe"))) yield return dir;
        }
...
public sealed class CoilAsmException(string message) : Exception(message);
